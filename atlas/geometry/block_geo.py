...
class BlockGeometry:
    centre:Vector2; rotation_deg:float; width:float; height:float
...
def compute_block(cx,cy,U,style=BLOCK_STYLE,rotation_deg=0.0):
    assert U>0
    W=style.width_ratio*U; H=style.height_ratio*U
    ...
    return BlockGeometry(
        centre=Vector2(cx,cy), rotation_deg=rotation_deg, width=W, height=H,
